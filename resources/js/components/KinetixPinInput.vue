...
const model = computed<string[]>(() => (props.value ?? "").split("").slice(0, length.value));
...
<template>
  ...
  >
    <PinInputInput
      ...
      :key="i"
      :index="i - 1"
      class="size-10 rounded-md border border-input bg-transparent text-center text-sm text-foreground shadow-xs transition-[color,box-shadow] outline-none focus:border-ring focus:ring-ring/50 focus:ring-[3px] dark:bg-input/30"
    />
  </PinInputRoot>
</template>
