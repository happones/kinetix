...
const formatted = computed(() => {
  ...
  try {
    ...
    return new Date(y, m - 1, d).toLocaleDateString(props.locale || undefined, {
      year: "numeric",
      month: "short",
      day: "numeric",
    });
  ...
  <PopoverRoot v-else v-model:open="open">
    ...
    <PopoverPortal>
      <PopoverContent
        align="start"
        :side-offset="4"
        class="z-50 w-auto p-0 outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95"
      >
        <KinetixCalendar
          :value="value"
          :locale="locale"
