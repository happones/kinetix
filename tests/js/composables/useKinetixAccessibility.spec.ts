...
describe("applyKinetixAccessibility", () => {
  ...
  it("maps text size to the right class and is exclusive", () => {
    ...
    expect(document.documentElement.classList.contains("kx-text-large")).toBe(true);
    ...
    expect(document.documentElement.classList.contains("kx-text-x-large")).toBe(false);
  });
  ...
  it("removes classes when preferences are turned off", () => {
    ...
    expect(document.documentElement.classList.contains("kx-reduce-motion")).toBe(false);
  });
});
