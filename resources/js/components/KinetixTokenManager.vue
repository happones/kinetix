...
<template>
  <div class="space-y-4">
    ...
    <div
      v-if="revealed"
      class="space-y-2 rounded-lg border border-border bg-muted/50 p-4"
    >
      ...
      <div class="flex items-center gap-2">
        <code
          class="min-w-0 flex-1 truncate rounded-md bg-background px-3 py-2 font-mono text-sm text-foreground"
          >{{ revealed }}</code
        >
        <button
          type="button"
    ...
    >
      <p v-if="tokens.length === 0" class="p-4 text-sm text-muted-foreground">
        {{ t("kinetix.no_tokens") }}
      </p>
