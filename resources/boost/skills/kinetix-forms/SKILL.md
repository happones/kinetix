...
- Weekly business hours: `BusinessHours::make('hours')` — per-day switch + HH:MM ranges + apply-to-all editor (Intl day names); defaults M–F 09:00–17:00; auto-validated (`kinetix_weekly_schedule` rule, also usable standalone). Pair with `AsWeeklySchedule` cast → `$model->hours?->isOpenAt(now($tz))` / `isOpenNow($tz)` / `effectiveSchedule()` / `rangesFor('monday')`; end ≤ start wraps overnight (fri 22:00–02:00 covers sat 01:00). Shape: `{monday: {enabled, ranges: [{start, end}]}, …}`, all 7 days always present.
...
For full details, reference the [Kinetix Forms Documentation](https://happones.github.io/kinetix/forms).
...
- **Select Option Mapping**: Use native Enum reflection mapping directly (e.g. `options(PostStatus::class)` + `->default(PostStatus::Draft->value)`) to easily bind Enums cast on models. Labels resolve from `getLabel()` when the enum implements `Happones\Kinetix\Support\Contracts\HasLabel` — or simply defines a public `getLabel(): ?string` method (duck-typed, so enums written against equivalent third-party label contracts work unchanged); enums without one fall back to the backing `value`/case `name`. Add the `Happones\Kinetix\Support\Concerns\HasLabelOptions` concern for a static `PostStatus::options()` array helper. Define the enum once: the same class drives `Select`/`Radio`, `TextColumn->badge()` (with `HasColor`/`HasIcon`), `SelectColumn`, `SelectFilter`, Infolist entries, and export columns.
