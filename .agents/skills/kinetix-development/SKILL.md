...
- **Serving a binary through an authenticated route: set the cache policy EXPLICITLY (REQUIRED)**: an authenticated response inherits the session's `no-cache, private`, and `response()->file()` answers `public` — so two paths serving the same asset disagree, one re-streaming the bytes on every view and the other inviting a shared proxy to hold authenticated content. Always `setPrivate()` + `setMaxAge()` + `setEtag()`/`setLastModified()` + `isNotModified($request)` (a 304 instead of the bytes), with a config TTL and `0` to opt out. `HelpController::cacheable()` is the reference. Rewritten `<img>` embeds also get `loading="lazy" decoding="async"` — each one is a round trip through PHP.
- **A directory the package creates under `storage/` ignores itself (REQUIRED)**: write `*\n!.gitignore\n` into it (never overwriting a host's existing file), exactly like Laravel's own `storage/framework/{cache,sessions,views,testing}`. Otherwise generated binaries are kept out of git only by whatever the host's root `.gitignore` happens to say. `HelpScreenshotsCommand::protectCaptureDirectory()` is the reference.
- **Generators write through `WritesGeneratedFiles` (REQUIRED)**: every `kinetix:make-*` command routes its writes through `putGenerated()` (`src/Commands/Concerns/WritesGeneratedFiles.php`), never a bare `File::put()`. A heredoc stub ends at its closing marker with NO trailing newline, so scaffolded files used to fail the HOST's first `pint` run on `single_blank_line_at_eof` — generated code arriving already broken by the host's own standards. The trait normalizes the tail so a stub author cannot forget. **A stub must also be Pint-clean in its own right**: imports in alpha order, no inline FQCN where an import exists (`use App\Models\Model;` + `Model::class`, not `\App\Models\Model::class`), no stray indentation on interpolated lines (a heredoc dedents its OWN lines only — interpolated content is inserted verbatim, so a multi-line `{$var}` needs the final indentation literally). `GeneratedScaffoldStyleTest` runs the real Pint over every generator's real output, which is the guard that catches the whole class instead of one rule at a time. `InstallCommand`'s append/edit paths are NOT generators — they manage their own newlines against the host's existing files; leave them alone.
...
- **Reka / shadcn only — NEVER native, NEVER invented styles (REQUIRED, no exceptions)**: Every control and every style traces back to Reka UI + shadcn-vue **new-york-v4** (reference: https://github.com/unovue/shadcn-vue/tree/dev/apps/v4/registry/new-york-v4/ui). Do not hand-roll widgets or class strings, and do not invent spacing/colors/shadows/variants — port shadcn's exactly.
  ...
  - **Text input / textarea / badge → native element + the canonical helper, never bespoke classes**: `inputClass` / `textareaClass` / `badgeVariants()` from `@/composables/useKinetixShadcnVariants`. Status badges/text/buttons → `useStatusColor` helpers (`statusBadgeClass`, …). There is NO `KinetixInput` (settled decision) — fields and badges are the token class strings above. Do not write `class="rounded-md border px-3 py-2 …"` by hand for a field, button, badge, or card; reuse the helper / `components/primitives/*` (Card, ScrollArea).
  - **Buttons → `KinetixButton` (REQUIRED for anything that fires a request)**: `components/KinetixButton.vue` is the shared base — `buttonVariants()` styling plus a built-in pending state: `loading` disables the button, swaps the `icon` slot for a spinner and sets `aria-busy`. Props `variant` / `size` / `type` (default `button`) / `loading` / `disabled`; slots `icon` + default label. Every action surface in the package routes through it (table toolbar, page header, confirm + upgrade modals, the record-modal and relation-manager footers, member provisioning, plan-lock CTA) and so do the `kinetix:make-resource` create/edit scaffolds, so double-click protection and pending feedback can't diverge per call site — per-button spinners come from `processingAction` (the in-flight action's name). `buttonVariants()` stays for elements that are NOT a `<button>` (`<a>`, Inertia `<Link>`, a reka trigger with `as-child`) and for controls that never enter a pending state (a picker's day cell). Contract documented in docs/actions.md §6.
...
- **Non-Reka elements via parity primitives (REQUIRED)**: For Card/Button/Badge/Input/ScrollArea (built "from scratch", not Reka), use Kinetix's own primitives that mirror shadcn-vue **new-york-v4** EXACTLY — Card family + `ScrollArea`/`ScrollBar` (Reka ScrollArea*) in `components/primitives/*` (Card v4 structure: `flex flex-col gap-6 py-6` card + `px-6` sections + `data-slot`; use `cn` from `./primitives/cn`), and `composables/useKinetixShadcnVariants` (`buttonVariants`/`badgeVariants`/`inputClass`/`textareaClass`). The DateTimePicker time columns use `ScrollArea`. Import these via RELATIVE paths (`./primitives/Card.vue`) — `@/components/ui/*` would resolve to the CONSUMER's files post-publish. Do NOT re-hand-roll `rounded-xl border ... p-6` card markup; reuse the primitives. shadcn Badge is a `rounded-full` pill; Kinetix soft status badges (`useStatusColor`) are a separate Filament-style element. No new deps (`cn` is a local 4-line join, not clsx/tailwind-merge). **Settled decision (do not revisit): Kinetix is self-contained — it does NOT import or re-publish the host's `@/components/ui` shadcn-vue files, and it does NOT ship a parallel public UI kit for fields, badges or cards (there is no `KinetixInput`; those are token class strings and `primitives/*`). The one shared element component is `KinetixButton` (the pending-state base above) — it WRAPS `buttonVariants()`, it does not fork the design line.** shadcn-vue is copy-paste (not an npm pkg): the starter kit ships only a SUBSET, with variant/version drift, so importing host ui = hard build failures for the consumer. We reuse shadcn's *foundation* — the same tokens (→ pixel-identical look + dark mode) and the same headless lib (Reka UI, a real declared peer dep) — not its copied files. The `primitives/` are an internal implementation detail, not "the UI".
- **Icons: guard on the RESOLVED component, never on the name (REQUIRED)**: `resolveIcon(name)` returns `null` for an unknown name, so `<component :is="resolveIcon(x)" v-if="x" />` renders an EMPTY element — and on an `->iconButton()` action that painted a button with no icon and no label (an icon button hides it): present, focusable, invisible. Always `v-if="resolveIcon(x)"`, and for an action button use **`isIconOnlyAction(action)`** (`composables/useKinetixIcons`) — not `action.isIconButton` — in all three places that must agree: the `buttonVariants` size, the `title`/`aria-label`, and whether the label renders. It is false when the icon does not resolve, so the button degrades to a normal labelled one. `KinetixPageHeader`/`KinetixActionDropdown`/`KinetixInfolistEntries` degrade differently, by design: a local wrapper falls back to `Circle`. `resolveIcon` warns once per unknown name in dev builds. **Host icons: `registerIcons({ 'my-name': MyIcon })`** from the host's `app.ts` (never published, so `kinetix:upgrade --force` can't drop it) — registrations WIN over the shipped map, which makes it the supported way to swap an icon too. There is deliberately no `kinetix.icons` config block: a name→string map resolved at runtime would pull all of Lucide into the bundle. New generic vocabulary goes INTO `ICONS`; app-specific names are the host's to register. Docs: docs/icons.md.
- **Disclosures → `KinetixCollapsible` (REQUIRED, never a hand-rolled `v-show`)**: `components/KinetixCollapsible.vue` is the shadcn-vue new-york-v4 Collapsible on Reka's `Collapsible*` — a real `<button>` with `aria-expanded`, and a HEIGHT animation (`height: auto` is not animatable, so the keyframes interpolate to `--reka-collapsible-content-height`, which Reka measures and publishes; the keyframes live in the component's own scoped style so the animation never depends on the host having declared shadcn's). Props `open`/`defaultOpen`/`title`/`summary`/`disabled`/`bare`, slots `default`/`trigger`. A collapsed section MUST keep a `summary` line: folded is not the same as unknown. Do NOT add per-component reduced-motion CSS — the `kinetixAccessibility` plugin already collapses every animation's duration with `!important` under BOTH `prefers-reduced-motion` and `kx-reduce-motion`. Reka suppresses `data-state` on the FIRST render (so content that starts open doesn't animate in) and its `aria-controls` lands from the first toggle onward (`contentId` is a non-reactive context value) — assert both accordingly. Verify with `npm run test:e2e:collapsible`; the DOM contract is pinned by `KinetixCollapsible.spec.ts`.
- **Grids inside a dialog use CONTAINER queries, not viewport breakpoints (REQUIRED)**: `sm:`/`lg:` measure the SCREEN, so a form in a fixed-width dialog gets a four-column layout on any wide monitor and wraps its labels. Give the wrapper `container-type: inline-size` in a scoped style and drive `grid-template-columns` from `@container` — the same mechanism as `kinetix-grid.css`/`useKinetixResponsiveGrid`. And never mix a bare checkbox into a label-over-field grid row: it has no label stack, so it never shares the row's baseline — give it its own row.
- **Drag-and-drop shared vocabulary (REQUIRED on every drag surface)**: never hand-roll drag feedback. Touch fallback = `useKinetixTouchDrag` (long-press floating clone). Drag visuals = `composables/kinetixDragStyles.ts` (`KINETIX_DRAG_SOURCE_CLASS` dims the origin item, `KINETIX_DROP_PREVIEW_CLASS` styles the in-flight item in a live-preview reorder). Drop placeholder = `KinetixDropGhost.vue` (dashed ghost labelled with the dragged item's title — kanban columns, calendar cells/slots; `compact` for chip-sized). List reorder = `useKinetixListReorder` (generic live-preview + commit-on-drop + revert-on-cancel; `useKinetixTableReorder` wraps it for tables, `KinetixMediaLibrary` uses it directly). New drag surfaces compose these; new feedback styles go INTO `kinetixDragStyles`, not inline.
- **Dialog shells + scrolling (REQUIRED on every modal/sheet surface)**: never hand-roll a dialog panel and never bound one with a raw `overflow-y-auto`. Shells are `components/primitives/KinetixModal.vue` (centered dialog) and `components/KinetixSheet.vue` (edge panel) — they own the focus trap, Escape, `aria-labelledby` and the bounded panel. The modal shell scrolls ITSELF (the fixed wrapper is the scroll container, the centering row carries `min-h-full`), so a form taller than the viewport can never strand its title or its actions off screen; `scroll-body` is the opt-in for the PINNED header/footer layout (max-h panel + scrolling body), never what keeps content reachable. Every scroller Kinetix owns inside a dialog goes through the `ScrollArea` primitive with `type="auto"` and a `-mr-3 pr-3` gutter (the reka viewport hides the native bar, so nothing else reserves that space) — raw `overflow-y-auto` stays only where ScrollArea genuinely can't serve: two-axis + `sticky` scrollers (role matrix, log payload blocks) and virtualizer-owned scroll elements (media grid). Actions belong in the shell's `#footer`; when the submit button sits outside the `<form>`, wire it with the native `form` attribute (`recordFormId` in `KinetixTable`, `attachFormId` in `KinetixRelationManager`). **Placement (REQUIRED for form dialogs):** `KinetixModal` takes `placement: 'center' | 'top'` (default `center` = the shadcn dialog). Every dialog the user WORKS in — a create/edit/view record modal, the relation-manager attach picker, the role editor, the announcement form, any `KinetixForm` in a modal — passes `placement="top"` (row gets `sm:items-start sm:pt-[10vh]`, mobile stays centered), because a centered panel re-centers on every height change (skeleton → form, validation errors, conditional fields) and moves the title under the user's eyes; anchored it only grows downward. With `scroll-body` the shell also caps the panel at `sm:max-h-[calc(90dvh-1rem)]` so the pinned footer can't fall below the fold — keep those two numbers in step if the offset ever changes. Confirmations, deletes and the upgrade modal stay `center`. `fullscreen` ignores placement. Verify geometry with `npm run test:e2e:modals` (Playwright, gallery `modal-scroll` specimen); the DOM contract is pinned by `KinetixModalShell.spec.ts` + `KinetixTableRecordModal.spec.ts`.
...
- **Host-model key columns (REQUIRED)**: columns referencing HOST models (`user_id`, `team_id`, morph ids, `*_by` ids) are built with `HostKeys::user()/team()/morph()` (`Happones\Kinetix\Support\HostKeys`) — NEVER a raw `unsignedBigInteger` — so they type themselves after the app's models (`kinetix.key_types` config). No DB-level foreign keys on them. When a new feature adds such a column: add the feature to the table in `docs/installation.md` § "UUID / ULID primary keys on your models" AND append the standard "UUID / ULID Host Models" note (with the feature's columns) to its Boost skill. Columns referencing Kinetix's own tables stay `unsignedBigInteger` and are never listed.
...
### Clickable rows (row click → view / edit)
- **Server decides, frontend obeys.** `Table::resolveRecordClick()` sets per row `TableRowData.recordUrl` + `recordUrlInNewTab` or `recordAction` (an action NAME the row renders — groups are flattened by `flattenRowActions()`). Inference when a channel is not set explicitly: first of `view` → `edit` that renders for the record; `isPlainLink()` (url, no inertiaVisit/httpRequest/dispatch/modal/download/preview/confirmation) → URL, anything else → action. URL wins over action. Explicit API: `recordUrl(?Closure, bool $shouldOpenInNewTab)`, `openRecordUrlInNewTab()`, `recordAction(string|Closure|null)`, `clickableRows(bool)` (tri-state, defers to `config('kinetix.tables.clickable_rows')`, default true). An explicit `recordAction` the row does not render is dropped (unauthorized ⇒ not clickable).
- **Frontend contract (REQUIRED for every row surface):** `composables/useKinetixRowClick.ts` is the ONLY row-activation logic — `KinetixTable` and `KinetixDataTable` both call `useKinetixRowClick({ runAction })`, where `runAction` is the SAME handler the row's own buttons use (`handleActionClick`), so a modal `view` from a row click goes through `useKinetixRecordModals` exactly like the button. Nested controls are inert via `NESTED_CONTROL_SELECTOR` (button/a/input/select/textarea/label/contenteditable/role=button|link|menuitem|checkbox|switch) AND the record-actions `<td>` carries `@click.stop` (belt and braces — the "⋯" trigger is a Reka `<button>`, its items live in a body portal). Clickable rows get `tabindex="0"`, `Enter` on the ROW ITSELF activates (`event.target === event.currentTarget`), `Ctrl/Cmd`+click → `window.open(url, '_blank', 'noopener,noreferrer')`. A `<tr>` cannot paint a box-shadow ring under `border-collapse`, so the focus indicator is `focus-visible:outline-2 -outline-offset-2 outline-ring/50` + the hover tint — do not swap it for the ring tokens. Tests: `TableRecordClickTest` (PHP inference + switches), `KinetixTableRowClick.spec.ts` (DOM contract incl. the "⋯" cell staying inert).
...
- **DatePicker / DateTimePicker default to the shadcn calendar** (`KinetixDatePicker` / `KinetixDateTimePicker`), NOT native inputs. `->native()` opts back to native `<input>`; `->locale()`, `DateTimePicker->minuteStep()`/`->twelveHour()` (AM/PM column). Inputs/textareas across forms reuse `inputClass`/`textareaClass` from `@/composables/useKinetixShadcnVariants`, buttons that fire a request go through `KinetixButton` — don't re-hand-roll field/button class strings.
...
- **Page action bars — `KinetixPageHeader` + `KinetixPageFooter`, both content-agnostic (v0.176.0)**: a page's two action bars take ONLY `actions: KinetixAction[]` (plus the header's `heading`/`description`), so anything can sit between them — a table, a form, or a fully custom component. `KinetixPageFooter` props: `actions`, `sticky` (default false; `position: sticky` NOT `fixed`, so the bar stays in layout and never covers the last of the content), `shortcuts` (default **false** — a footer usually repeats actions the header already bound, and two handlers on one chord is a bug); slots `before-actions` (save state / hint) + default. Its row is `flex-col-reverse` below `sm` (last action = primary, on top, full width — a column flex container stretches its children, which is why there is NO `items-center` at that width) and a right-aligned row above, matching the dialog shells' footers. **Both bars render through `KinetixActionBar.vue`** (props `actions`/`shortcuts`/`stack`, slots `before`/`after`) — the ONE implementation of the action row: dropdown groups, `requiresConfirmation()` modal, `->shortcut()` registration, per-button pending via `processingAction`, `viewType: 'link'` vs button, icon position. Add a new action-rendering surface by composing `KinetixActionBar`, never by copying its template. NOTE: `Table::footerActions()` is a DIFFERENT thing — it renders inside the table's own footer card, not at page level. Tests: `KinetixPageBars.spec.ts`.
...
Queue-backed CSV/Excel import (a three-step wizard) + export (download notification). Excel via `phpoffice/phpspreadsheet`; CSV native.

- **Import**: extend `Importer` (`getColumns()` of `ImportColumn`, `$model`, `resolveRecord()` for upsert, `importRow()`, `chunkSize()`/`queue()`). `Importer::guessMapping($headers)` auto-maps headers→columns (normalized, **collision-free**). Endpoints `imports/upload|preview|start` (importer class + stored file are encrypted tokens). `ImportProcessor` (ShouldQueue) maps by header index, validates column `rules()`, chunked transactions, deletes temp file, sends a completion notification. **Template (v0.81.0)**: `Importer` `protected bool $downloadableTemplate = true` + `protected ?string $templateFileName` (default studly class basename `.csv`); accessors `hasDownloadableTemplate()`/`getTemplateFileName()`/`getTemplateHeaders()` (column labels — auto-map on upload). `GET imports/template?importer=token` (`ImportController::template`, streamDownload CSV, 404 when disabled/invalid). `ImportAction::importer()` adds `template` (filename|null) to the `open-importer` dispatch detail; `KinetixImportModal` forwards it as the `template` prop of `KinetixImporter`, which renders the download `<a>` under the dropzone. i18n `download_template`. Tests in `ImportStartTest` + `KinetixImporter.spec.ts`. **Tenancy (v0.69.0)**: `Importer::context(Request $request): array` is captured by `ImportController::start()` at dispatch, serialized as the job's `$context`, and restored via `$importer->withContext()` before any row — read `$this->context`/`getContext()` inside `importRow()`/`resolveRecord()`. NEVER infer the tenant in the worker (`Team::first()` = cross-tenant leak). UI: the wizard, below.
- **Bounded reads — the scalability contract (REQUIRED, v0.174.0)**: nothing in the import path may scale with the file (a 1M-row upload is a supported case). `FileReader` is the single place that reads a file and it is bounded three ways: `read($path, $options, $limit)` **stops at `$limit`** (preview = `previewRows` rows, so previewing is O(limit) not O(file)); `stream($path, $options)` is a **Generator** of data rows and is what `ImportProcessor` consumes, so the job holds one `chunkSize()` chunk and never the file; `countRows()` counts WITHOUT parsing (CSV = `substr_count` over 1 MB `fread` blocks; spreadsheet = `listWorksheetInfo()['totalRows']`, zero cells loaded) and is documented as a cheap label, not an exact contract. `headers($path, $options)` reads one record. Spreadsheets have no streaming reader, so they are read in **windows** of `kinetix.imports.spreadsheet_chunk_size` rows via `RowWindowFilter` (`IReadFilter`) + `setLoadSheetsOnly()`, re-opening the file per window — **first worksheet only** (deterministic; previously `getActiveSheet()`). `ImportProcessor` writes failed rows to the downloadable CSV **as they fail** (`openFailureLog`/`logFailedRow`/`publishFailureLog`) instead of collecting them. **NEVER call `FileReader::read()` without a limit on a user file, and never `array_chunk` rows you collected yourself** — that is the exact bug this replaced (`handle()` used to `read()` the whole file into an array).
- **Import dialog settings (v0.174.0)**: `config('kinetix.imports')` — `max_upload_size` (KB, enforced on the upload endpoint), `preview`, `preview_rows`, `preview_columns` (0 = no cap), `layout`, `fullscreen_threshold`, `spreadsheet_chunk_size` — each overridable per importer via `protected ?bool $preview` / `?int $previewRows` / `?int $previewColumns` / `?string $layout` / `?int $fullscreenThreshold` / `?int $maxUploadSize`, resolved by `hasPreview()`/`getPreviewRows()`/`getPreviewColumns()`/`getLayout()`(unknown → `auto`)/`getFullscreenThreshold()`/`getMaxUploadSize()` and serialized by `settings(): ImportSettingsData`. The settings ride BOTH the `open-importer` dispatch detail (so the shell sizes itself before a file exists) and every `ImportPreviewData`. `ImportController::upload()` resolves + authorizes the importer BEFORE validating the file, so the ceiling can come from the importer and an unauthorized caller never stores an upload.
- **`Importer::isExactMatch($headers, $mapping)` (v0.174.0)**: true when every target column found a header AND every non-blank header was claimed — always true for a file filled in from the downloadable template, and the wizard then **skips the mapping step**. It deliberately fails on an unclaimed source column (that data would be silently dropped); the mapping step names those columns instead.
- **The wizard (v0.174.0)**: `KinetixImporter.vue` is an orchestrator over `composables/useKinetixImporter.ts` (state machine: `file` → `mapping` → `review`) plus `components/Importer/*` — `ImporterSteps` (`<ol>` + `aria-current="step"`, completed steps are buttons), `ImporterDropzone` (drop target that IS a real file input), `ImporterOptions` (parse options **collapsed by default**, summary line while folded, emits an `update` patch — never mutates the prop), `ImporterMapping` (searchable/filterable list, `KinetixLabel` per select, progress counter, names unclaimed source columns), `ImporterPreview` (row cap from the server + column cap with a show-all toggle, `overflow-x-auto` on ITS OWN container, `<caption>` + `scope` on every header). **Why a wizard**: a single panel held the options, one select per column and a full-width preview at once, so a 20+ column file grew the dialog past the viewport and stranded its own actions.
- **`surface` decides who scrolls (REQUIRED when embedding)**: `inline` (the page — the default, for a page-level wizard), `modal` (the wizard's own ScrollArea, capped `max-h-[min(55vh,32rem)]`), `fullscreen` (its ScrollArea fills the panel), `sheet` (none — `KinetixSheet` already scrolls its body, so adding one nests scrollers).
- **Dialog escalation (v0.174.0)**: `KinetixImportModal` picks the shell. Under `layout: 'auto'` it promotes the **same** `KinetixModal` to the new `fullscreen` prop past `fullscreenThreshold` source columns (the wizard emits `update:columns`) — same component, so it is RESIZED, never remounted, and the user's mapping survives. `'sheet'` is an explicit per-importer opt-in only, because swapping shells mid-flow unmounts the wizard and loses the file. `KinetixModal`'s `fullscreen` = `h-[calc(100dvh-2rem)]` + the `scrollBody` grid rows + `min-h-0` on the body (so a child can fill it); it does not change any existing modal. Tests: `ImportScalabilityTest`, `ImportPreviewTest`, `KinetixImporter.spec.ts`, `KinetixImporterSurfaces.spec.ts`.
...
Every icon is declared from PHP by NAME and resolved by one shared frontend map
(`composables/useKinetixIcons`). Docs: [docs/icons.md]; boost skill:
`kinetix-icons`.

- **`resolveIcon(name)`** → the registered map first, then the shipped `ICONS`, then `null`. Case-insensitive. Warns ONCE per unknown non-empty name in dev builds (`import.meta.env.DEV`), naming `registerIcons` as the fix.
- **`registerIcons({ 'name': Component })`** (v0.175.0) — the host's extension point, called once from ITS `app.ts` (a file Kinetix never publishes, so `--force` can't drop it). Registrations OVERRIDE shipped names, which makes this the supported way to swap an icon as well. Components, not strings, so the host's bundler still tree-shakes — the reason there is no `kinetix.icons` config block. `registeredIconNames()` lists everything resolvable, sorted, for a host parity test.
- **`isIconOnlyAction(action)`** — the predicate three call sites must agree on (button size variant, `title`/`aria-label`, label `v-if`). False when the icon does not resolve, so an `->iconButton()` degrades to a normal labelled button instead of painting nothing. Used by `KinetixTable`, `KinetixDataTable` and `useKinetixCalendarEventDetails`.
- **The shipped map covers generic product vocabulary** — CRUD/table chrome, status, people & org, commerce (`store`/`printer`/`receipt`/`truck`/`banknote`/`hand-coins`/`shopping-basket`), data & docs, trades. App-specific names are the host's to register; new GENERIC names belong in `ICONS`.
- **`kinetix:doctor`** names this hatch specifically when `useKinetixIcons.ts` shows local edits (`DoctorCommand::DRIFT_HATCHES`) — generic "use a wrapper, a slot, or config" advice was wrong for a file whose only pre-0.175 escape was a patch.
- Tests: `useKinetixIcons.spec.ts`, `KinetixActionIconFallback.spec.ts`, `DoctorCommandTest`.
...
- **Plan-lock UI (v0.167.0) — REQUIRED shared vocabulary for "this needs a better plan"**: never hand-roll a padlock/upsell block. `<KinetixPlanLock>` owns every presentation via `variant`: `card` (dashed lock card replacing the content), `overlay` (content stays visible, `opacity-40` + optional `blur-[2px]`, `aria-hidden` + `inert`, lock panel centred over it), `banner` (row strip), `badge` (content dimmed + padlock appended, `@click.capture` swallows the click and opens the dialog). Copy props `featureName`/`title`/`description`/`ctaLabel`/`badgeLabel`, behaviour props `modal`/`blur`/`upgradeUrl`; app-wide defaults from `kinetix.billing.lock` (`variant`/`modal`/`blur`/`badge_label`) shared as `kinetix_billing.lock` — per-instance props win. NO gating prop = unconditional upsell; `#locked` slot gets `{ remaining, open }`. Gating logic lives ONCE in `useKinetixPlanAccess()` (`useKinetixPlan.ts`) — `<KinetixPlanFeature>`, `<KinetixPlanLock>` and (through the lock) `<KinetixPlanGate>` all route through it, so a new gate composes these instead of re-deriving `canUseFeature`/`hasReachedLimit`. Sub-parts `components/Billing/PlanLockPanel|PlanLockCta.vue` (the CTA renders NOTHING without an upgrade URL — no dead-end buttons); `<KinetixUpgradeModal>` (on `KinetixModal`) is the shared upsell dialog and is also usable standalone via `v-model:open`. Keys `plan_locked_*`/`plan_upgrade_*` ×7 locales. Tests `KinetixPlanLock.spec.ts`, `BillingSharedPlanTest`.
- **Blank is not an id (REQUIRED wherever a Stripe identifier is read, v0.168.1)**: use `filled()`/`blank()`, never `!== null`. `''` is what a form default / import / `fill()` leaves in a column and forwarding it produces an opaque Stripe error far from the cause. `BillingManager::hasStripeCustomer()` is stricter than Cashier's `hasStripeId()` (plain null check); `ensureStripeCustomer()` NULLS a blank `stripe_id` before creating, else Cashier throws `CustomerAlreadyCreated` and the billable is stuck forever. Blank `payment_method` = none given (`subscribe()` normalizes it, `addPaymentMethod('')` throws); blank plan price = no price for that cycle (`Plan::stripePriceId()` returns null); blank subscription price NEVER matches a plan in `HasPlan::currentPlan()` — plans seeded with `''` price columns would all match and grant the wrong features (a gating bug, not just ergonomics). Tests: `BillingManagerTest` blank-string section, `HasPlanTest::test_a_blank_subscription_price_never_matches_a_plan`.
...
- **Hardening (v0.172.0)** — from a multi-tenant HR platform running 18 sources:
  - **`query()` scopes BOTH drivers (was Critical).** It used to be honored on the LIKE branch and dropped on the Scout one, so a model adopting `Searchable` for an unrelated reason silently made every tenant's records searchable by every other tenant — green build, correct-looking UI. Scout now only *proposes*: `fetchViaScout()` takes `->keys()` and hydrates them through `($this->queryResolver)()->whereKey($keys)`, re-imposing the engine's order in PHP (`whereKey` returns rows in whatever order the DB likes, discarding the relevance ranking). **Never "fix" this by hydrating with `$model::search()->get()`** — that is the leak. Regression test: `SpotlightScoutTest::test_the_source_query_scopes_the_scout_path_too`.
  - **`->scoutWhere([col => val])`** → `Scout\Builder::where()`, so the engine's buffer is spent on rows that survive `query()`. Constraining only the hydration is safe but under-fills. Both, not either.
  - **`spotlight.throttle`** (default `'60,1'`, null removes) on the route — one request fans out to every authorized source. **Testbench needs `cache.default = array`** or the limiter dies on the missing `cache` table.
  - **`spotlight.min_chars`** (default 2) enforced in `SpotlightResource::search()` — NOT in the controller: `''`/short queries must still reach the in-memory `SpotlightLink`s ("empty query → links only" is documented behavior). Shared to the client via `kinetix_config.spotlight.min_chars` so the palette's gate agrees; `SpotlightController::minChars()` is public for host sources.
  - **`->trustQuery()`** skips the per-record `view` pass; otherwise `search()` walks up to `MAX_AUTHORIZATION_PAGES` (5) pages of `limit` instead of one fixed `limit * 3` buffer, so a rejecting policy under-fills only when the set is genuinely exhausted. The DB path now `orderBy`s the qualified key — paging without an order repeats and skips rows.
  - **`spotlight.limit` is read** (`resolveLimit()`, `$limit` is `?int` now); it was published config nothing consumed.
  - **`->priority(int)`** via the OPTIONAL `HasSpotlightPriority` interface (adding it to `SpotlightSource` would fatal every host implementation on upgrade). Controller sorts groups by it desc, stable. DB driver also floats prefix matches within a group (`rank()`); Scout results are left in engine order.
  - **`SpotlightRegistry` memoizes the class-name scan** (`discovered`, invalidated by `discover()`), NOT the instances — a source reads request state when built (locale for its heading, tenant for its query).
  - **Frontend**: `kinetixFetch` gained `signal?: AbortSignal` + `isKinetixAbort()`; `useKinetixSpotlight().search()` aborts the previous request, guards with a sequence id, and resolves **`null` when superseded** (callers must skip the assignment, not paint it) — responses used to land out of order and the stale one won. Exposes `loading` + `minChars`; the palette renders searching / keep-typing / empty as three distinct states.
- `laravel/scout` is a dev dep + `suggest`. Tests: `SpotlightTest` (database driver, per-record policy, source ability, endpoint, min_chars + config, throttle middleware, config limit, paged buffer, trustQuery, priority, prefix rank), `SpotlightScoutTest` (scout `collection` driver + the tenancy regression + scoutWhere), `SpotlightDiscoveryTest` (scan memoized once via a `File::partialMock` spy, instances still rebuilt), `useKinetixSpotlight.spec.ts`, `useKinetixHttp.spec.ts`.
- Full guide: `docs/spotlight.md` (incl. a "Scaling profile of the LIKE driver" section: `%term%` indexes nothing, but a source query filtering an indexed tenant column first confines the scan to that tenant's slice — cost tracks the LARGEST TENANT, not total rows; dotted relation columns add a `whereHas` subquery each).
...
- **Checklist variants (v0.170.0)**: `KinetixOnboardingChecklist` is now a thin shell — it loads the state, computes `percent`/`progressText`/`visible`, and dispatches through a component map (no `v-if` chain) to `components/Onboarding/OnboardingChecklistCard.vue` (`variant="card"`, default) or `OnboardingChecklistSidebar.vue` (`variant="sidebar"`). Both are presentational and emit `complete(key)` / `dismiss`.
  - The **sidebar** variant is sized for a shadcn `<SidebarFooter>`: terse `onboarding_progress_short` counter + `h-1` bar, no step descriptions, icon-only `×` dismiss. Each row keeps BOTH affordances without nesting interactives — the leading circle is a `<button>` (`onboarding_mark_done_step` aria-label, `size-6` = WCAG 2.2's 24×24 target) and the title is a **stretched link** (`after:absolute after:inset-0` on the `<a>`, `relative` on the `<li>`, `relative z-10` on the button) so the rest of the row follows the CTA. Completed rows are inert and struck through.
  - `group-data-[collapsible=icon]:hidden` on the root is shadcn's own sidebar contract — the block folds away when the rail collapses to icons, and the class never matches outside a sidebar. Dark surface is `dark:bg-muted/40`, NOT `bg-card`: shadcn's `--sidebar` and `--card` are the same value in dark, so a plain card reads as a bare outline on the rail.
  - **Shared payload (v0.171.0)**: `Inertia::share('kinetix_onboarding', …)` ships `OnboardingManager::for($user)` on every response when `onboarding.enabled` && `onboarding.share` (default true) && authenticated — same pattern as `kinetix_announcements`. `useKinetixOnboarding` overlays a local `ref` on the shared prop (`state = computed(() => local ?? shared)`), so `load()` short-circuits when the payload is there (`load(true)` forces) and `complete`/`dismiss` results win until the next response. **`progressFor()` is now `firstOrNew`, not `firstOrCreate`** — `for()` runs on EVERY request, so a read must not write a row. Cost to call out to hosts: every `completedUsing` callback runs per response.
  - a11y additions applied to BOTH variants: the progress bar gained an `aria-label` + `aria-valuetext`, every decorative icon is `aria-hidden`, and each row carries an `sr-only` `onboarding_status_completed|pending` so completion isn't signalled by the strike-through alone. New keys: `onboarding_progress_short`, `onboarding_mark_done_step`, `onboarding_status_completed`, `onboarding_status_pending` (x7 locales).
- Tests: `OnboardingTest` (auto vs manual completion, persistence, per-user isolation, dismiss, 401, shared payload present/absent, read writes no row), `useKinetixOnboarding.spec.ts` (payload hydration, forced refetch, local-wins), `KinetixOnboardingChecklist.spec.ts` (both variants: counter, collapse class, stretched link, tick button, sr-only status, progressbar naming), `useKinetixTour.spec.ts`. Gallery: `onboarding-checklist`, `onboarding-checklist-sidebar` (framed in a mock nav rail) and `onboarding-checklist-sidebar-collapsed` (the same rail expanded next to one carrying `group` + `data-collapsible="icon"` — the collapse contract is proven by rendering, not asserted).
...
## 49. Kinetix Help Center (in-app manual from markdown, v0.168.0 localized)

Markdown files in a HOST-owned directory (`kinetix.help.path`, default `resources/help`) become permission-gated articles with search, TOC, screenshots and Spotlight results. Slug = filename minus `.md` (`NN-` prefix orders); FLAT front matter only (`title`/`group`/`icon`/`order`/`permission` — no YAML dep); `README.md`/`_*.md` ignored. Rendering: `Str::markdown` with `html_input=strip` + `allow_unsafe_links=false`; `<!-- kinetix:can x -->…<!-- /kinetix:can -->` blocks stripped on the RAW markdown pre-render (the renderer eats comments); a gated ARTICLE 404s and vanishes from index+search (never 403 — no existence leak).

- **Locale is a first-class ARGUMENT, never ambient (REQUIRED pattern for any content-by-language feature)**: `HelpManager::articles/find/render/search/localizedPath(..., ?string $locale = null)`; `locale(?string $requested)` validates against `supportedLocales()` (`help.locales` → Locale module's locales → variants discovered on disk + app/fallback locale) so untrusted input can neither pick files nor widen the cache keyspace. Resolution chain `{slug}.pt_BR.md` → `{slug}.pt.md` → `{slug}.{help.fallback_locale}.md` → `{slug}.md`, walked ONCE by `resolveFile()` (returns path + the locale actually written). `HelpArticle` carries `locale` + `isFallback`; `help.hide_untranslated` drops fallbacks from index/search/show. Endpoints take `?locale=` and answer with `Content-Language` + `Vary` (URL varies per language → no cache can cross-serve). `show` also returns `requestedLocale` + `availableLocales`.
- **Per-locale index (perf)**: `index(locale)` builds metadata AND the plain-text search corpus in ONE read per article (search used to re-read every file per keystroke: 2N reads → 0). Memoized per request keyed `locale@fingerprint` (mtimes) so a long-lived worker can't serve a stale index; `help.cache.strategy` = `fingerprint` (default; instant invalidation) or `ttl` (no stat per request, `Cache::remember` owns expiry — no memo). Manager bound as a SINGLETON in `registerHelp()` so endpoints + Spotlight share the index. Rendered HTML is NEVER cached (per-user gating).
- **Screenshots per language**: `render()` rewrites `src="screenshots/x.png"` to the streaming route WITH `?locale=` (the article's own locale); the controller tries `{prefix}/{locale}/{name}` → `{prefix}/{name}` on the disk, then `{help.path}/screenshots/{locale}|/{name}` locally. `kinetix:help-screenshots --locale=es` stores a localized set.
- **Vue (published)**: `KinetixHelpCenter` (grouped cards/list + 250ms debounced server search) and `KinetixHelpArticle` (body + TOC + prev/next). Both follow the app language THROUGH `useKinetixHelp`, which sends `?locale=`, caches per language client-side (module-level Maps + `clearKinetixHelpCache()` — specs must call it plus `enableAutoUnmount`, since each instance watches the locale) and **re-fetches on a locale switch** (an Inertia `router.reload()` re-renders the page component WITHOUT re-running `onMounted`, so a mount-only fetch left the manual stale — never fetch localized content only on mount). Untranslated content is marked, never silently swapped: `lang` attribute + badge on the index, `lang`/`dir="auto"` + notice on the article, and chips (`availableLocales`) to read ONE article in another language without touching the app locale (`hide-language-switcher` opts out). TOC anchors are Unicode-aware (`\p{L}\p{N}` + Latin-only diacritic folding — an ASCII `\w` rule erases Cyrillic/CJK/Arabic headings into identical ids; Arabic marks are NOT stripped, `إ` is not a decorated `ا`).
- **Commands**: `kinetix:make-help-page` (pages + sample article; `--locale=es [--from=slug]` scaffolds variants with front matter verbatim + heading skeleton + TODO markers), `kinetix:help-status [--locale=* --strict]` (coverage matrix, CI gate), `kinetix:help-screenshots [--only --locale --keep-local]`. Tests `HelpTest`, `HelpCommandsTest`, `KinetixHelpCenter.spec.ts`, `KinetixHelpArticle.spec.ts`, `useKinetixHelpToc.spec.ts`. Skill `kinetix-help`; guide `docs/help-center.md`.
