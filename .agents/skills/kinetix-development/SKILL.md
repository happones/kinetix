...
- Tests: `CalendarTest` (ISO datetime + timezone shift/closure + allDay detection + description), `KinetixEventCalendar.spec.ts` (backward-compat month grid, timezone correctness across an ahead-timezone day boundary, view switcher, week/day hourly positioning + all-day banner, modal/sheet/suppressed popup, anchorDate), `KinetixSheet.spec.ts`. Full guide: `docs/calendar.md`.
