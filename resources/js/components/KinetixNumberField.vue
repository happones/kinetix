...
const props = withDefaults(
  ...
  { value: null, config: null, disabled: false, placeholder: null, compact: false },
);
...
<template>
  <NumberFieldRoot
    ...
    class="inline-flex items-center rounded-md border border-input bg-transparent shadow-xs transition-[color,box-shadow] focus-within:border-ring focus-within:ring-ring/50 focus-within:ring-[3px] dark:bg-input/30"
    :class="[
      compact ? 'h-8 w-32 text-xs' : 'h-9 w-full text-sm',
      disabled ? 'cursor-not-allowed opacity-50' : '',
    ]"
    @update:model-value="onUpdate"
  >
    <NumberFieldDecrement
      class="flex h-full items-center justify-center px-2 text-muted-foreground transition-colors hover:text-foreground disabled:opacity-40"
    >
      <Minus :class="compact ? 'size-3' : 'size-4'" />
    </NumberFieldDecrement>
    <NumberFieldInput
      :placeholder="placeholder ?? ''"
      class="min-w-0 flex-1 bg-transparent text-center text-foreground outline-none tabular-nums"
    />
    <NumberFieldIncrement
      class="flex h-full items-center justify-center px-2 text-muted-foreground transition-colors hover:text-foreground disabled:opacity-40"
    >
