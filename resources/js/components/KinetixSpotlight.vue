...
function onSelect(item: unknown): void {
  ...
  if (selected.url) {
    ...
    window.dispatchEvent(new CustomEvent(selected.event, { detail: selected }));
  }
}
</script>
...
<template>
  <DialogRoot :open="open" @update:open="onOpenChange">
    <DialogPortal>
      ...
      <DialogContent
        class="fixed left-1/2 top-24 z-50 w-full max-w-lg -translate-x-1/2 overflow-hidden rounded-xl border border-border bg-popover text-popover-foreground shadow-lg"
      >
        <VisuallyHidden>
          <DialogTitle>{{ t("kinetix.spotlight_placeholder") }}</DialogTitle>
          <DialogDescription>{{
        ...
        >
          ...
          <ComboboxContent class="max-h-80 overflow-y-auto p-2">
            <ComboboxEmpty
            ...
            >
              ...
              <ComboboxItem
                ...
                :value="item"
                class="flex cursor-default flex-col gap-0.5 rounded-md px-2 py-2 text-sm text-foreground outline-none data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground"
              >
