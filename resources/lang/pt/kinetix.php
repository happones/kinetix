<?php

declare(strict_types=1);

return [
    'notifications'                     => 'Notificações',
    'mark_all_as_read'                  => 'Marcar todas como lidas',
    'clear_all'                         => 'Limpar tudo',
    'no_notifications'                  => 'Sem notificações',
    'notifications_appear_here'         => 'As novas notificações aparecerão aqui.',
    'just_now'                          => 'Agora mesmo',
    'minutes_ago'                       => 'Há :minutes min',
    'hours_ago'                         => 'Há :hours h',
    'delete'                            => 'Excluir',
    'search_records'                    => 'Pesquisar registros...',
    'filters'                           => 'Filtros',
    'table_filters'                     => 'Filtros da Tabela',
    'reset'                             => 'Limpar',
    'all'                               => 'Todos',
    'enable_filter'                     => 'Habilitar filtro',
    'toggle_columns'                    => 'Alternar colunas',
    'columns'                           => 'Colunas',
    'per_page'                          => 'Por página:',
    'showing_records'                   => 'Mostrando :from a :to de :total resultados',
    'no_records'                        => 'Sem resultados',
    'page_of'                           => 'Página :current de :total',
    'page_number'                       => 'Página :current',
    'pagination'                        => 'Paginação',
    'first_page'                        => 'Primeira página',
    'previous_page'                     => 'Página anterior',
    'next_page'                         => 'Próxima página',
    'last_page'                         => 'Última página',
    'select_row'                        => 'Selecionar linha',
    'actions'                           => 'Ações',
    'reorder'                           => 'Reordenar',
    'results_count'                     => ':count resultados',
    'move_up'                           => 'Mover para cima',
    'move_down'                         => 'Mover para baixo',
    'row_moved'                         => 'Movido para a posição :position de :total',
    'kanban_keyboard_hint'              => 'Pressione a seta esquerda ou direita para mover este cartão para outra coluna',
    'kanban_moved_to'                   => 'Movido para :column',
    'showing_range'                     => 'Mostrando :from a :to',
    'no_records_found'                  => 'Nenhum registro encontrado.',
    'copy'                              => 'Copiar',
    'reveal'                            => 'Mostrar',
    'hide'                              => 'Ocultar',
    'confirm'                           => 'Confirmar',
    'cancel'                            => 'Cancelar',
    'close'                             => 'Fechar',
    'confirm_heading'                   => 'Tem certeza?',
    'key'                               => 'Chave',
    'value'                             => 'Valor',
    'add_row'                           => 'Adicionar linha',
    'add_item'                          => 'Adicionar item',
    'more_actions'                      => 'Mais ações',
    'upload'                            => 'Enviar',
    'csv_options'                       => 'Opções de CSV',
    'delimiter'                         => 'Colunas delimitadas por',
    'enclosure'                         => 'Texto delimitado por',
    'omit_lines'                        => 'Omitir as primeiras N linhas',
    'has_header'                        => 'O arquivo tem linha de cabeçalho',
    'apply'                             => 'Aplicar',
    'column_mapping'                    => 'Mapeamento de colunas',
    'not_mapped'                        => '— Não mapeado —',
    'rows_detected'                     => ':count linhas detectadas',
    'start_import'                      => 'Iniciar importação',
    'download_template'                 => 'Baixar modelo',
    'importing'                         => 'Importando…',
    'import_step_file'                  => 'Arquivo',
    'import_step_mapping'               => 'Mapeamento',
    'import_step_review'                => 'Revisar',
    'import_dropzone_hint'              => 'Arraste o arquivo aqui ou escolha um',
    'import_dropzone_formats'           => 'CSV, TSV, XLS ou XLSX · até :size',
    'import_remove_file'                => 'Remover arquivo',
    'import_parse_options'              => 'Opções de leitura',
    'import_parse_options_help'         => 'Afetam apenas como o arquivo é lido — mude-as se a pré-visualização parecer errada.',
    'import_reparse'                    => 'Reler arquivo',
    'import_delimiter_comma'            => 'Vírgula ( , )',
    'import_delimiter_semicolon'        => 'Ponto e vírgula ( ; )',
    'import_delimiter_tab'              => 'Tabulação',
    'import_delimiter_pipe'             => 'Barra vertical ( | )',
    'import_enclosure_double'           => 'Aspas duplas ( " )',
    'import_enclosure_single'           => 'Aspas simples ( \' )',
    'import_enclosure_none'             => 'Nenhuma',
    'import_has_header_short'           => 'com cabeçalho',
    'import_no_header_short'            => 'sem cabeçalho',
    'import_skipped_lines_short'        => ':count linhas omitidas',
    'import_back'                       => 'Voltar',
    'import_continue'                   => 'Continuar',
    'import_mapping_search'             => 'Buscar campos',
    'import_mapping_unmapped_only'      => 'Apenas não mapeados',
    'import_mapping_reset'              => 'Restaurar sugestões',
    'import_mapping_progress'           => ':mapped de :total campos mapeados',
    'import_mapping_missing_required'   => ':count campos obrigatórios sem mapear',
    'import_mapping_no_matches'         => 'Nenhum campo corresponde à sua busca.',
    'import_exact_match'                => 'O arquivo corresponde exatamente a este importador — todos os campos foram mapeados automaticamente.',
    'import_required_field'             => 'Obrigatório',
    'import_unnamed_column'             => 'Coluna :number',
    'import_unused_columns'             => 'Sem mapeamento, serão ignoradas (:count): :columns',
    'import_preview_sample'             => 'Amostra das primeiras :rows linhas · :columns de :total colunas',
    'import_preview_show_all_columns'   => 'Mostrar mais :count colunas',
    'import_preview_show_fewer_columns' => 'Mostrar menos colunas',
    'import_preview_caption'            => 'Linhas de amostra do arquivo enviado, com o campo mapeado para cada coluna.',
    'import_preview_row_heading'        => 'Linha',
    'import_preview_ignored'            => 'ignorada',
    'import_preview_empty'              => 'O arquivo não tem linhas de dados para pré-visualizar.',
    'import_summary_file'               => 'Arquivo',
    'import_summary_rows'               => 'Linhas',
    'import_summary_mapped'             => 'Campos mapeados',
    'import_summary_ignored'            => 'Colunas ignoradas',
    'import_queued_help'                => 'A importação roda em segundo plano. Você será notificado ao terminar, com a lista das linhas ignoradas.',
    'import_parsed_announce'            => 'Arquivo lido: :rows linhas e :columns colunas detectadas.',
    'import_started'                    => 'Importação na fila. Você será notificado ao concluir.',
    'export_started'                    => 'Exportação na fila. Você será notificado ao concluir.',
    'import_failed'                     => 'Algo deu errado. Tente novamente.',
    'import_complete'                   => 'Importação concluída',
    'import_complete_body'              => ':imported importadas, :failed ignoradas',
    'export_ready'                      => 'Exportação pronta',
    'export_ready_body'                 => 'Sua exportação foi concluída e está pronta para download.',
    'download_export'                   => 'Baixar',
    'choose_file'                       => 'Escolha um arquivo ou arraste-o aqui',
    'uploading'                         => 'Enviando…',
    'remove'                            => 'Remover',
    'upload_failed'                     => 'Falha no envio. Tente novamente.',
    'min'                               => 'Mín',
    'max'                               => 'Máx',
    'edit'                              => 'Editar',
    'view'                              => 'Ver',
    'create'                            => 'Criar',
    'download'                          => 'Baixar',
    'preview'                           => 'Visualizar',
    'export'                            => 'Exportar',
    'import'                            => 'Importar',
    'restore'                           => 'Restaurar',
    'force_delete'                      => 'Excluir permanentemente',
    'attach'                            => 'Vincular',
    'relation_loading'                  => 'Carregando…',
    'detach'                            => 'Desvincular',
    'attached'                          => 'Vinculado.',
    'detached'                          => 'Desvinculado.',
    'detach_confirm'                    => 'Desvincular este registro? O registro em si não é excluído.',
    'attach_none_found'                 => 'Nenhum registro para vincular.',
    'associate'                         => 'Associar',
    'dissociate'                        => 'Desassociar',
    'associated'                        => 'Registros associados.',
    'dissociated'                       => 'Registros desassociados.',
    'dissociate_confirm'                => 'Desassociar este registro? O registro em si não é excluído.',
    'associate_none_found'              => 'Nenhum registro para associar.',
    'view_any'                          => 'Ver todos',
    'delete_any'                        => 'Excluir vários',
    'trashed_filter'                    => 'Registros excluídos',
    'with_trashed'                      => 'Com excluídos',
    'only_trashed'                      => 'Apenas excluídos',
    'selected'                          => ':count selecionados',
    'clear_selection'                   => 'Limpar seleção',

    // Módulo de cobrança
    'billing_subscription_updated'   => 'Assinatura atualizada com sucesso.',
    'billing_payment_method_added'   => 'Método de pagamento adicionado com sucesso.',
    'billing_payment_method_removed' => 'Método de pagamento removido com sucesso.',
    'billing_subscription_cancelled' => 'Assinatura cancelada com sucesso.',
    'billing_subscription_resumed'   => 'Assinatura retomada com sucesso.',
    'billing_feature_unavailable'    => 'Seu plano atual não inclui este recurso.',
    'entitlement_unavailable'        => 'Este recurso não está disponível.',
    'entitlement_forbidden'          => 'Você não tem permissão para fazer isto.',
    'plan_limit_reached'             => 'O limite de :key do seu plano (:limit) foi atingido.',
    'validation_weekly_schedule'     => 'O campo :attribute não é um horário semanal válido.',
    'business_hours_add_range'       => 'Adicionar horário',
    'business_hours_remove_range'    => 'Remover horário',
    'business_hours_apply_all'       => 'Aplicar a todos os dias',
    'business_hours_closed'          => 'Fechado',
    'plan_locked_title'              => 'Upgrade necessário',
    'plan_locked_body'               => 'Este recurso não está incluído no seu plano atual.',
    'plan_upgrade'                   => 'Fazer upgrade',
    'plan_locked_feature'            => 'O recurso ":feature" não está incluído no seu plano atual.',
    'plan_locked_hint'               => 'Bloqueado — upgrade necessário',
    'plan_upgrade_modal_title'       => 'Faça upgrade para desbloquear',
    'plan_upgrade_modal_body'        => 'Este recurso está disponível nos nossos planos premium. Faça upgrade da sua assinatura para desbloqueá-lo.',
    'plan_upgrade_dismiss'           => 'Talvez mais tarde',

    // Rótulos de UI de cobrança
    'billing_current_plan'             => 'Plano atual',
    'billing_upgrade'                  => 'Fazer upgrade',
    'billing_switch_plan'              => 'Trocar de plano',
    'billing_per_month'                => '/ mês',
    'billing_per_year'                 => '/ ano',
    'billing_monthly'                  => 'Mensal',
    'billing_yearly'                   => 'Anual',
    'billing_subscription_status'      => 'Status da assinatura',
    'billing_status'                   => 'Status',
    'billing_expires_on'               => 'Expira em',
    'billing_no_subscription'          => 'Nenhuma assinatura ativa encontrada.',
    'billing_cancel_subscription'      => 'Cancelar assinatura',
    'billing_resume_subscription'      => 'Retomar assinatura',
    'billing_trial'                    => 'Avaliação',
    'billing_trial_ends_on'            => 'A avaliação termina em',
    'billing_trial_active'             => 'Você está em um período de avaliação gratuita até :date.',
    'billing_trial_active_plan'        => 'Você está testando o plano :plan até :date.',
    'billing_trial_badge'              => 'Avaliação de :days dias',
    'billing_trial_notice_title'       => 'Nenhum método de pagamento necessário',
    'billing_trial_notice_description' => 'Você pode iniciar sua avaliação sem inserir um cartão. Você só será cobrado quando a avaliação terminar e escolher um plano pago.',
    'billing_trial_already_used'       => 'Você já utilizou sua avaliação gratuita. Adicione um método de pagamento para assinar.',
    'billing_invoices'                 => 'Faturas',
    'billing_date'                     => 'Data',
    'billing_total'                    => 'Total',
    'billing_no_invoices'              => 'Ainda não há faturas.',
    'billing_payment_method'           => 'Forma de pagamento',
    'billing_payment_method_desc'      => 'Escolha um cartão existente ou adicione um novo.',
    'billing_add_card'                 => 'Adicionar cartão',
    'billing_add_payment_method'       => 'Adicionar forma de pagamento',
    'billing_default'                  => 'Padrão',
    'billing_payment_required'         => 'Adicione uma forma de pagamento antes de assinar um plano pago.',
    'billing_secure_payments'          => 'Pagamentos seguros',
    'billing_secure_payments_desc'     => 'Utilizamos o Stripe para processamento seguro de pagamentos. As informações do seu cartão de crédito nunca passam pelos nossos servidores.',
    'billing_usage_title'              => 'Uso deste período',
    'billing_usage_over_limit'         => 'Você atingiu o limite incluído para este uso.',

    // Seletores de data e pré-visualização
    'pick_date'            => 'Escolher data',
    'pick_month'           => 'Escolher mês',
    'pick_year'            => 'Escolher ano',
    'pick_week'            => 'Escolher semana',
    'pick_date_range'      => 'Escolher intervalo de datas',
    'pick_time'            => 'Escolher horário',
    'picker_now'           => 'Agora',
    'picker_done'          => 'Concluir',
    'picker_hours'         => 'Horas',
    'picker_minutes'       => 'Minutos',
    'picker_this_month'    => 'Este mês',
    'picker_this_week'     => 'Esta semana',
    'picker_this_year'     => 'Este ano',
    'week_of'              => 'Semana :week, :year',
    'datetime_placeholder' => 'DD/MM/AAAA hh:mm',
    'preview_unavailable'  => 'Pré-visualização indisponível para este tipo de arquivo.',

    // Address picker & filter
    'address_line1'   => 'Endereço linha 1',
    'address_line2'   => 'Endereço linha 2',
    'address_city'    => 'Cidade',
    'address_state'   => 'Estado / Província',
    'address_postal'  => 'Código postal',
    'address_country' => 'País',
    'address_search'  => 'Buscar endereço…',

    // Rich text editor
    'editor_write'          => 'Escrever',
    'editor_preview'        => 'Pré-visualização',
    'editor_tiptap_missing' => 'O editor Tiptap não está registrado. Instale @tiptap/core + @tiptap/starter-kit e registre-os no seu entry com registerKinetixTiptap() (veja a doc de Forms), ou troque este campo para o editor básico ou markdown.',
    'signature_clear'       => 'Limpar',

    // Comments
    'comments_title'            => 'Comentários',
    'comment_placeholder'       => 'Escreva um comentário…',
    'comment_reply_placeholder' => 'Escreva uma resposta…',
    'comment_post'              => 'Publicar',
    'comment_reply'             => 'Responder',
    'comment_empty'             => 'Ainda não há comentários. Seja o primeiro a comentar.',
    'comment_deleted'           => 'Comentário excluído.',
    'comment_edited'            => 'editado',
    'comment_just_now'          => 'agora mesmo',
    'comment_unknown_author'    => 'Alguém',
    'comment_invalid_parent'    => 'Você só pode responder a um comentário de primeiro nível.',

    // Tags
    'tag_placeholder' => 'Adicionar uma etiqueta…',
    'tag_remove'      => 'Remover etiqueta',

    // Notification preferences
    'notification_prefs_title'       => 'Preferências de notificação',
    'notification_prefs_description' => 'Escolha como deseja ser notificado.',
    'notification_prefs_type'        => 'Notificação',
    'notification_prefs_empty'       => 'Nenhum tipo de notificação configurado.',

    // Saved views
    'saved_views_label'       => 'Visualizações',
    'saved_view_save_current' => 'Salvar visualização atual',
    'saved_view_name'         => 'Nomeie esta visualização',
    'saved_view_saved'        => 'Visualização salva.',
    'saved_view_deleted'      => 'Visualização excluída.',
    'saved_view_empty'        => 'Ainda não há visualizações salvas.',
    'saved_view_default'      => 'Definir como padrão',
    'saved_view_delete'       => 'Excluir visualização',

    // Kanban
    'kanban_empty'         => 'Solte cartões aqui',
    'kanban_card'          => 'Cartão arrastável',
    'kanban_keyboard_hint' => 'Use as setas para a esquerda e para a direita para mover este cartão para a coluna anterior ou seguinte.',
    'kanban_move_failed'   => 'Não foi possível mover o cartão. Tente novamente.',

    // Event calendar
    'calendar_today'      => 'Hoje',
    'calendar_prev'       => 'Mês anterior',
    'calendar_next'       => 'Próximo mês',
    'calendar_prev_week'  => 'Semana anterior',
    'calendar_next_week'  => 'Próxima semana',
    'calendar_prev_day'   => 'Dia anterior',
    'calendar_next_day'   => 'Próximo dia',
    'calendar_more'       => '+:count mais',
    'calendar_view_month' => 'Mês',
    'calendar_view_week'  => 'Semana',
    'calendar_view_day'   => 'Dia',
    'calendar_all_day'    => 'Dia inteiro',
    'calendar_no_events'  => 'Sem eventos',
    'calendar_view_event' => 'Ver detalhes',

    'calendar_move_failed'   => 'Não foi possível mover o evento. Tente novamente.',
    'calendar_moved_to'      => 'Movido para :date',
    'calendar_keyboard_hint' => 'Segure Alt e pressione uma seta para mover este evento',
    'calendar_invalid_date'  => 'A data de destino não é válida.',

    // Announcements
    'announcements_title' => 'Novidades',
    'announcements_empty' => 'Ainda não há anúncios.',
    'announcements_new'   => 'Novo',

    'announcements_level_info'           => 'Info',
    'announcements_level_feature'        => 'Novidade',
    'announcements_level_fix'            => 'Correção',
    'announcements_previous'             => 'Anúncio anterior',
    'announcements_next'                 => 'Próximo anúncio',
    'announcements_dismiss'              => 'Dispensar',
    'announcements_pause'                => 'Pausar a rotação',
    'announcements_play'                 => 'Retomar a rotação',
    'announcements_slide_position'       => '{current} de {total}',
    'announcements_go_to'                => 'Mostrar: {title}',
    'announcements_manage_title'         => 'Anúncios',
    'announcements_new_entry'            => 'Novo anúncio',
    'announcements_empty_hint'           => 'Publique sua primeira novidade — ela aparece no feed “Novidades” de todos.',
    'announcements_status_draft'         => 'Rascunho',
    'announcements_status_scheduled'     => 'Agendado',
    'announcements_status_published'     => 'Publicado',
    'announcements_global'               => 'Da plataforma',
    'announcements_global_readonly'      => 'Da plataforma — edite fora de uma equipe',
    'announcements_field_title'          => 'Título',
    'announcements_field_body'           => 'Mensagem',
    'announcements_field_level'          => 'Nível',
    'announcements_field_published_at'   => 'Publicar em',
    'announcements_field_published_hint' => 'Deixe vazio para manter como rascunho; uma data futura agenda.',
    'announcements_field_expires_at'     => 'Expira em',
    'announcements_field_expires_hint'   => 'Deixe vazio para mantê-lo no feed para sempre.',
    'announcements_status_expired'       => 'Expirado',
    'announcements_delete_confirm'       => 'Excluir este anúncio?',
    'unread_count'                       => '{count} não lidos',

    // Papéis e permissões
    'save'                        => 'Salvar',
    'select_all'                  => 'Selecionar tudo',
    'search_permissions'          => 'Buscar permissões…',
    'role_name'                   => 'Nome do papel',
    'role_members'                => '{count} membros',
    'role_permissions_count'      => '{count} permissões',
    'role_matrix_hint'            => 'Ative permissões por módulo. Clique no nome do módulo para alternar a linha inteira.',
    'role_matrix_module'          => 'Módulo',
    'permission_matrix'           => 'Matriz de permissões',
    'role_matrix_full'            => 'Todas as habilidades concedidas',
    'role_matrix_partial'         => '{granted} de {total} habilidades',
    'role_matrix_none'            => 'Nenhuma habilidade concedida',
    'role_more_features'          => '+{count} mais',
    'role_global'                 => 'Global',
    'role_global_hint'            => 'Esta função aplica-se a todas as equipes; somente um superadministrador pode modificá-la.',
    'role_global_create_label'    => 'Função global (todas as equipes)',
    'role_global_create_hint'     => 'Visível em todas as equipes. Somente superadministradores poderão modificá-la depois.',
    'role_delete_members_warning' => 'A função :role ainda está atribuída a :count membro(s). A exclusão fica bloqueada até reatribuí-los.',
    'access'                      => 'Acesso',
    'role_custom_abilities'       => 'Permissões personalizadas',
    'tour_next'                   => 'Próximo',
    'tour_previous'               => 'Voltar',
    'tour_done'                   => 'Concluído',
    'tour_progress'               => '{current} de {total}',
    'help_center'                 => 'Central de ajuda',
    'help_search_placeholder'     => 'Pesquisar no manual…',
    'help_no_results'             => 'Nenhum artigo corresponde a “{query}”.',
    'help_no_articles'            => 'Ainda não há artigos de ajuda.',
    'help_grid_view'              => 'Visualização em cartões',
    'help_list_view'              => 'Visualização em lista',
    'help_on_this_page'           => 'Nesta página',
    'help_previous'               => 'Anterior',
    'help_next'                   => 'Próximo',
    'help_not_found'              => 'Este artigo não está disponível.',
    'help_translation_missing'    => 'Este artigo ainda não foi traduzido — exibindo a versão em :language.',
    'help_read_in'                => 'Ler em',
    'help_untranslated'           => 'Ainda não traduzido',
    'roles_title'                 => 'Papéis e permissões',
    'create_role'                 => 'Novo papel',
    'no_roles'                    => 'Ainda não há papéis.',
    'confirm_delete'              => 'Excluir?',
    'saved'                       => 'Salvo.',
    'save_failed'                 => 'Não foi possível salvar.',
    'action_failed'               => 'Não foi possível concluir a ação.',
    'record_created'              => 'Registro criado com sucesso.',
    'record_restored'             => 'Registro restaurado com sucesso.',
    'record_force_deleted'        => 'Registro excluído permanentemente.',
    'record_updated'              => 'Registro atualizado com sucesso.',
    'record_deleted'              => 'Registro excluído com sucesso.',
    'deleted'                     => 'Excluído.',
    'delete_failed'               => 'Não foi possível excluir.',

    // Membros e provisionamento
    'members_title'               => 'Membros',
    'member_email'                => 'E-mail',
    'member_username'             => 'Usuário',
    'member_phone'                => 'Telefone',
    'member_role'                 => 'Papel',
    'member_provision'            => 'Adicionar membro',
    'member_resend'               => 'Reenviar',
    'member_revoke'               => 'Remover',
    'no_members'                  => 'Ainda não há membros.',
    'member_status_pending'       => 'Pendente',
    'member_status_active'        => 'Ativo',
    'member_status_revoked'       => 'Revogado',
    'member_provisioned'          => 'Convite enviado.',
    'member_provision_failed'     => 'Não foi possível adicionar o membro.',
    'member_role_updated'         => 'Papel atualizado.',
    'member_revoked'              => 'Membro removido.',
    'member_revoke_confirm'       => 'Remover :email? A função será removida e o membro marcado como revogado.',
    'member_search'               => 'Buscar membros…',
    'show_more'                   => 'Mostrar mais',
    'activation_title'            => 'Defina sua senha',
    'activation_name'             => 'Seu nome',
    'activation_password'         => 'Senha',
    'activation_password_confirm' => 'Confirmar senha',
    'activation_submit'           => 'Ativar conta',

    // E-mails de associação
    'member_activation_subject'  => 'Ative sua conta no :app',
    'member_activation_intro'    => 'Você foi adicionado ao :app. Defina uma senha para ativar sua conta.',
    'member_activation_button'   => 'Ativar conta',
    'member_activation_expiry'   => 'Este link expirará em breve. Se já expirou, peça a um administrador para reenviá-lo.',
    'member_activation_sms'      => 'Ative sua conta :app: :url',
    'password_expired'           => 'Sua senha expirou. Escolha uma nova.',
    'password_must_change'       => 'Você precisa definir uma nova senha antes de continuar.',
    'password_previously_used'   => 'Você não pode reutilizar uma das suas últimas :count senhas.',
    'password_current_incorrect' => 'A senha atual está incorreta.',
    'password_changed'           => 'Sua senha foi atualizada.',
    'password_change_title'      => 'Altere sua senha',
    'password_current'           => 'Senha atual',
    'password_new'               => 'Nova senha',
    'password_confirm'           => 'Confirme a nova senha',
    'password_change_submit'     => 'Atualizar senha',
    'password_expires_in'        => 'Sua senha expira em :days dia(s).',
    'password_history_hint'      => 'Você não pode reutilizar suas últimas :count senhas.',

    // Configurações
    'settings_saved' => 'Configurações salvas.',

    // Registro de atividade
    'activity_event_created' => 'Criado',
    'activity_event_updated' => 'Atualizado',
    'activity_event_deleted' => 'Excluído',
    'activity_by'            => 'por :name',
    'activity_system'        => 'Sistema',
    'activity_empty'         => 'Ainda não há atividade.',
    'activity_load_more'     => 'Carregar mais',

    // Personificação
    'impersonate'         => 'Personificar',
    'impersonating'       => 'Você está personificando :name',
    'impersonation_leave' => 'Voltar à sua conta',

    // Spotlight
    'spotlight_placeholder' => 'Buscar…',
    'spotlight_empty'       => 'Sem resultados.',
    'spotlight_searching'   => 'Buscando…',
    'spotlight_min_chars'   => 'Digite ao menos :count caracteres',

    // Webhooks
    'webhooks_title'       => 'Webhooks',
    'webhook_add'          => 'Adicionar endpoint',
    'webhook_name'         => 'Nome',
    'webhook_url'          => 'URL de destino',
    'webhook_events'       => 'Eventos',
    'webhook_active'       => 'Ativo',
    'webhook_inactive'     => 'Inativo',
    'webhook_test'         => 'Testar',
    'webhook_rotate'       => 'Girar segredo',
    'webhook_logs'         => 'Registros',
    'no_webhooks'          => 'Ainda não há endpoints.',
    'webhook_test_queued'  => 'Evento de teste enfileirado.',
    'webhook_secret_shown' => 'Segredo de assinatura (exibido uma vez): :secret',
    'refresh'              => 'Atualizar',
    'logs_webhooks_tab'    => 'Entregas de webhooks',
    'logs_api_tab'         => 'Requisições API',
    'logs_search'          => 'Pesquisar…',
    'logs_result_all'      => 'Todas',
    'logs_result_success'  => 'Sucesso',
    'logs_result_failed'   => 'Falhas',
    'logs_status'          => 'Status',
    'logs_event'           => 'Evento',
    'logs_endpoint'        => 'Endpoint',
    'logs_attempt'         => 'Tentativa',
    'logs_time'            => 'Data',
    'logs_request'         => 'Requisição',
    'logs_token'           => 'Token',
    'logs_duration'        => 'Duração',
    'logs_empty'           => 'Nenhum registro ainda.',
    'logs_total'           => '{total} registros',
    'logs_detail'          => 'Detalhe do registro',
    'logs_payload'         => 'Payload',
    'logs_response'        => 'Resposta',
    'logs_redeliver'       => 'Reenviar',
    'logs_request_body'    => 'Corpo da requisição',
    'logs_response_body'   => 'Corpo da resposta',

    // Tokens de desenvolvedor
    'tokens_title'           => 'Tokens de API',
    'token_add'              => 'Criar token',
    'token_name'             => 'Nome do token',
    'token_name_placeholder' => 'ex.: Deploy CI',
    'token_scopes'           => 'Escopos',
    'token_full_access'      => 'Este token terá acesso total.',
    'token_expires_at'       => 'Data de expiração (opcional)',
    'token_created'          => 'Criado',
    'pdf_reset'              => 'Restaurar padrões',
    'pdf_preview_hint'       => 'Pré-visualização com dados de exemplo — idêntica ao PDF final.',
    'token_expires_hint'     => 'O token deixa de funcionar no final desse dia. Vazio = sem expiração.',
    'token_expires'          => 'Expira {date}',
    'token_expired'          => 'Expirado',
    'token_create'           => 'Criar',
    'no_tokens'              => 'Ainda não há tokens.',
    'token_revoke'           => 'Revogar',
    'token_revoked'          => 'Token revogado.',
    'token_copy'             => 'Copiar',
    'token_copied'           => 'Token copiado para a área de transferência.',
    'token_dismiss'          => 'Concluído',
    'token_reveal_notice'    => 'Copie seu novo token agora. Você não poderá vê-lo novamente.',
    'token_last_used'        => 'Último uso',
    'token_never'            => 'Nunca',

    // Connected accounts / social auth
    'connected_accounts_title'         => 'Contas conectadas',
    'connected_accounts_description'   => 'Vincule um provedor para entrar mais rápido.',
    'continue_with'                    => 'Continuar com :provider',
    'connected_account_connect'        => 'Conectar',
    'connected_account_disconnect'     => 'Desconectar',
    'connected_account_connected'      => 'Conectada',
    'connected_account_not_connected'  => 'Não conectada',
    'connected_account_none_available' => 'Nenhum provedor disponível.',
    'connected_account_linked'         => 'Conta vinculada.',
    'connected_account_unlinked'       => 'Conta desconectada.',
    'connected_account_taken'          => 'Essa conta já está vinculada a outro usuário.',
    'connected_account_failed'         => 'Não foi possível conectar essa conta. Tente novamente.',
    'connected_account_last_method'    => 'Você não pode desconectar seu único método de login. Defina uma senha primeiro.',
    'password_set_title'               => 'Definir uma senha',
    'password_set_description'         => 'Você entra com uma conta conectada. Defina uma senha para usar também e-mail e senha.',
    'password_change_title'            => 'Alterar senha',
    'password_change_description'      => 'Atualize a senha que você usa para entrar.',
    'password_current'                 => 'Senha atual',
    'password_new'                     => 'Nova senha',
    'password_confirm'                 => 'Confirmar senha',
    'password_updated'                 => 'Senha atualizada.',

    // Browser sessions / device management
    'sessions_title'                 => 'Sessões do navegador',
    'sessions_description'           => 'Gerencie e encerre suas sessões ativas em outros dispositivos.',
    'sessions_unavailable'           => 'Os detalhes da sessão exigem o driver de sessão de banco de dados.',
    'sessions_empty'                 => 'Nenhuma sessão ativa.',
    'session_this_device'            => 'Este dispositivo',
    'session_active_now'             => 'Ativa agora',
    'sessions_logout_others'         => 'Encerrar outras sessões',
    'sessions_logout_others_confirm' => 'Digite sua senha para confirmar o encerramento das suas outras sessões em todos os seus dispositivos.',
    'sessions_logged_out_others'     => 'Suas outras sessões foram encerradas.',

    // Integração
    'onboarding_title'            => 'Primeiros passos',
    'onboarding_progress'         => ':completed de :total concluídos',
    'onboarding_progress_short'   => ':completed de :total',
    'onboarding_dismiss'          => 'Dispensar',
    'onboarding_mark_done'        => 'Marcar como concluído',
    'onboarding_mark_done_step'   => 'Marcar :step como concluído',
    'onboarding_go'               => 'Ir',
    'onboarding_status_completed' => 'Concluído',
    'onboarding_status_pending'   => 'Pendente',

    // Tour do produto
    'tour_next' => 'Próximo',
    'tour_back' => 'Voltar',
    'tour_skip' => 'Pular',
    'tour_done' => 'Concluir',

    // GDPR autosserviço
    'gdpr_export_title'           => 'Baixe seus dados',
    'gdpr_export_desc'            => 'Solicite uma cópia dos seus dados pessoais. Avisaremos com um link de download quando estiver pronta.',
    'gdpr_export_button'          => 'Solicitar exportação',
    'gdpr_export_queued'          => 'Sua exportação está sendo preparada. Você será notificado quando estiver pronta.',
    'gdpr_export_failed'          => 'Não foi possível iniciar a exportação. Tente novamente.',
    'gdpr_export_ready'           => 'Sua exportação de dados está pronta',
    'gdpr_export_ready_body'      => 'A exportação dos seus dados pessoais foi concluída e está pronta para download.',
    'gdpr_delete_title'           => 'Excluir sua conta',
    'gdpr_delete_desc'            => 'Exclui permanentemente sua conta e seus dados pessoais. Esta ação não pode ser desfeita.',
    'gdpr_delete_button'          => 'Excluir conta',
    'gdpr_delete_confirm_heading' => 'Excluir sua conta?',
    'gdpr_delete_confirm_body'    => 'Isto remove permanentemente sua conta e seus dados pessoais. Esta ação não pode ser desfeita.',
    'gdpr_password'               => 'Confirme sua senha',
    'gdpr_password_incorrect'     => 'A senha está incorreta.',
    'gdpr_delete_failed'          => 'Não foi possível excluir sua conta. Tente novamente.',

    // Acessibilidade
    'toggle_theme'              => 'Alternar tema',
    'appearance_light'          => 'Claro',
    'appearance_dark'           => 'Escuro',
    'appearance_system'         => 'Sistema',
    'language'                  => 'Idioma',
    'teams_switch'              => 'Trocar de equipe',
    'teams_select'              => 'Selecione uma equipe',
    'teams_new'                 => 'Nova equipe',
    'breadcrumb_create'         => 'Criar',
    'breadcrumb_edit'           => 'Editar',
    'presence_online'           => '{count} on-line',
    'queue_title'               => 'Saúde das filas',
    'queue_throughput'          => 'Por minuto',
    'queue_recent'              => 'Trabalhos recentes',
    'queue_pending'             => 'Pendentes',
    'queue_failed'              => 'Falhos',
    'queue_retry'               => 'Repetir',
    'queue_wait'                => '{seconds}s de espera',
    'queue_unavailable'         => 'As métricas de filas estão indisponíveis.',
    'queue_status_running'      => 'Ativo',
    'queue_status_paused'       => 'Pausado',
    'queue_status_inactive'     => 'Inativo',
    'health_title'              => 'Saúde do sistema',
    'health_unavailable'        => 'As verificações de saúde estão indisponíveis.',
    'health_status_ok'          => 'Saudável',
    'health_status_warning'     => 'Aviso',
    'health_status_failed'      => 'Com falhas',
    'table_repeater_empty'      => 'Ainda não há linhas.',
    'mail_new'                  => 'Novo modelo',
    'mail_disabled'             => 'Off',
    'mail_name'                 => 'Nome',
    'mail_key'                  => 'Chave',
    'mail_subject'              => 'Assunto',
    'mail_format'               => 'Formato',
    'mail_body'                 => 'Corpo',
    'mail_body_hint'            => 'Escreva em Markdown ou HTML. Referencie variáveis pela chave entre chaves duplas.',
    'mail_variables'            => 'Variáveis',
    'mail_var_key'              => 'chave',
    'mail_var_sample'           => 'Valor de exemplo',
    'mail_preview'              => 'Pré-visualização',
    'mail_test_email'           => 'Endereço de e-mail',
    'mail_send_test'            => 'Enviar teste',
    'rating_out_of'             => 'de :max',
    'rating_reviews'            => 'Com base em :total avaliações',
    'chart_empty'               => 'Nenhum dado disponível',
    'period_today'              => 'Hoje',
    'period_yesterday'          => 'Ontem',
    'period_7d'                 => 'Últimos 7 dias',
    'period_30d'                => 'Últimos 30 dias',
    'period_90d'                => 'Últimos 3 meses',
    'period_month'              => 'Este mês',
    'period_year'               => 'Este ano',
    'period_all'                => 'Todo o período',
    'media_add'                 => 'Adicionar arquivos',
    'media_uploading'           => 'Enviando…',
    'media_upload_failed'       => 'Alguns arquivos não puderam ser enviados.',
    'report_mail_intro'         => 'Seu relatório “:name” está em anexo.',
    'report_mail_outro'         => 'Enviado automaticamente por :app.',
    'a11y_title'                => 'Acessibilidade',
    'a11y_description'          => 'Ajuste a interface às suas necessidades. As alterações são salvas na sua conta.',
    'a11y_text_size'            => 'Tamanho do texto',
    'a11y_text_normal'          => 'Normal',
    'a11y_text_large'           => 'Grande',
    'a11y_text_x_large'         => 'Muito grande',
    'a11y_reduced_motion'       => 'Reduzir movimento',
    'a11y_reduced_motion_help'  => 'Minimiza animações e transições.',
    'a11y_high_contrast'        => 'Aumentar contraste',
    'a11y_high_contrast_help'   => 'Contornos e sublinhados mais fortes para legibilidade.',
    'a11y_underline_links'      => 'Sublinhar links',
    'a11y_underline_links_help' => 'Sublinha sempre os links, não apenas ao passar o cursor.',
    'a11y_enhanced_focus'       => 'Foco aprimorado',
    'a11y_enhanced_focus_help'  => 'Mostra um contorno de foco mais espesso e sempre visível.',
    'skip_to_content'           => 'Pular para o conteúdo',

    // Resumos de tabela
    'summary_total' => 'Total',

    // Assistente (wizard)
    'wizard_next'    => 'Próximo',
    'wizard_back'    => 'Voltar',
    'wizard_finish'  => 'Concluir',
    'wizard_step_of' => 'Etapa :current de :total',

    // Atalhos de teclado
    'shortcuts_title' => 'Atalhos de teclado',

    // Seletor de fuso horário
    'timezone_placeholder'        => 'Selecione um fuso horário…',
    'timezone_search_placeholder' => 'Buscar fusos horários…',
    'timezone_empty'              => 'Nenhum fuso horário encontrado.',
    'timezone_clear'              => 'Limpar',
    'timezone_region_africa'      => 'África',
    'timezone_region_america'     => 'América',
    'timezone_region_antarctica'  => 'Antártida',
    'timezone_region_arctic'      => 'Ártico',
    'timezone_region_asia'        => 'Ásia',
    'timezone_region_atlantic'    => 'Atlântico',
    'timezone_region_australia'   => 'Austrália',
    'timezone_region_europe'      => 'Europa',
    'timezone_region_indian'      => 'Oceano Índico',
    'timezone_region_pacific'     => 'Pacífico',

    // Cookie consent
    'cookie_consent_message'     => 'Usamos cookies para melhorar sua experiência.',
    'cookie_consent_policy_link' => 'Saiba mais',
    'cookie_consent_accept'      => 'Aceitar',
    'cookie_consent_decline'     => 'Recusar',

    // Reports Center
    'report_run_status_pending'     => 'Pendente',
    'report_run_status_running'     => 'Em execução',
    'report_run_status_completed'   => 'Concluído',
    'report_run_status_failed'      => 'Falhou',
    'report_run_status_cancelled'   => 'Cancelado',
    'report_run_ready'              => 'Seu relatório está pronto',
    'report_run_ready_body'         => 'O relatório solicitado terminou de ser gerado.',
    'report_runs_title'             => 'Execuções de relatórios',
    'report_runs_report_column'     => 'Relatório',
    'report_runs_rows_column'       => 'Linhas',
    'report_runs_loading'           => 'Carregando execuções…',
    'report_launcher_title'         => 'Relatórios',
    'report_launcher_empty'         => 'Ainda não há tipos de relatório registrados.',
    'report_launched'               => 'Relatório na fila — acompanhe o progresso abaixo.',
    'report_launch_failed'          => 'Não foi possível iniciar o relatório.',
    'run_now'                       => 'Executar agora',
    'cancel_run'                    => 'Cancelar',
    'retry_run'                     => 'Tentar novamente',
    'report_schedules_title'        => 'Relatórios agendados',
    'report_schedule_frequency'     => 'Frequência',
    'report_schedule_create'        => 'Agendar',
    'report_schedule_created'       => 'Agendamento de relatório criado.',
    'report_schedule_create_failed' => 'Não foi possível criar o agendamento do relatório.',

    // Confidential Fields
    'confidential_unlock'             => 'Desbloquear',
    'confidential_lock'               => 'Bloquear',
    'confidential_password_label'     => 'Senha',
    'confidential_password_incorrect' => 'Senha incorreta.',

    // Table write endpoints (inline edits, reordering, kanban moves)
    'table_column_not_editable' => 'Esta coluna não é editável.',
    'table_record_not_found'    => 'Registro não encontrado.',
    'table_write_forbidden'     => 'Você não tem autorização para esta ação.',
    'table_invalid_signature'   => 'Assinatura de tabela inválida.',
    'table_invalid_model'       => 'Classe de modelo inválida.',
    'table_descriptor_expired'  => 'A sessão desta tabela expirou. Recarregue a página.',
    'table_not_reorderable'     => 'Esta tabela não permite reordenação.',
    'kanban_invalid_status'     => 'Status inválido.',

    // Import / export / upload endpoints
    'import_invalid'                  => 'Importador inválido.',
    'import_invalid_session'          => 'Sessão de importação inválida.',
    'import_forbidden'                => 'Você não tem permissão para executar esta importação.',
    'import_required_columns_missing' => 'Colunas obrigatórias não foram mapeadas.',
    'export_invalid'                  => 'Exportador inválido.',
    'export_forbidden'                => 'Você não tem permissão para executar esta exportação.',
    'upload_invalid_field'            => 'Campo de upload inválido.',
    'upload_invalid_path'             => 'Caminho de arquivo inválido.',
    'upload_blocked_type'             => 'Este tipo de arquivo não é permitido.',

    // Job failures and form submit
    'submit'                        => 'Enviar',
    'export_failed'                 => 'A exportação falhou',
    'export_failed_body'            => 'Não foi possível gerar sua exportação. Tente novamente.',
    'import_failed_body'            => 'Não foi possível processar sua importação. Tente novamente.',
    'import_failed_row'             => 'Linha :row: :reason',
    'import_failed_title'           => 'A importação falhou',
    'export_failed_rows'            => ':count linhas não puderam ser exportadas e foram ignoradas.',
    'download_failed_rows'          => 'Baixar linhas com falha',
    'import_failures_row_heading'   => 'Linha',
    'import_failures_error_heading' => 'Erro',
    'import_failed_more'            => '…e :count linhas adicionais falharam.',
    'gdpr_export_failed_body'       => 'Não conseguimos preparar a exportação dos seus dados. Solicite novamente.',
    'search_invalid_token'          => 'Token de busca inválido.',
    'search_invalid_model'          => 'Modelo de busca inválido.',
    'search_forbidden'              => 'Esta busca não está autorizada.',
    'search_expired'                => 'A sessão de busca expirou. Recarregue a página.',

    // Reports center, repeater rows and permission guardrails
    'repeater_row_not_found'              => 'Linha não encontrada.',
    'form_session_expired'                => 'A sessão deste formulário expirou. Recarregue a página.',
    'report_invalid'                      => 'Relatório inválido.',
    'report_run_not_cancellable'          => 'Esta execução já não pode ser cancelada.',
    'report_run_not_retryable'            => 'Esta execução não pode ser repetida.',
    'permissions_cannot_grant_unheld'     => 'Você não pode conceder permissões que não possui: :permissions',
    'permissions_would_revoke_own_access' => 'Esta alteração revogaria o seu próprio acesso à gestão de funções.',
    'permissions_role_in_use'             => 'A função :role ainda está atribuída a :count membro(s) desta equipe. Reatribua-os antes de excluí-la.',
];
