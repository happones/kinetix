...
- **The reading options are a `KinetixCollapsible`**: the shared shadcn/Reka disclosure (a real `<button>` with `aria-expanded`, animating its measured height), collapsed by default and keeping a `summary` line while folded — folded is not the same as unknown. Reuse it for any disclosure; never hand-roll a `v-show` toggle. Its field grid uses a **container query**, not `sm:`/`lg:`: the dialog's width does not follow the screen, so viewport breakpoints squeezed the fields into columns their labels could not fit.
