  ...
  >
    <div class="flex flex-wrap items-center gap-0.5 border-b border-border bg-muted/40 p-1">
      <button
        ...
        type="button"
        :disabled="disabled"
        class="inline-flex size-7 items-center justify-center rounded text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground disabled:pointer-events-none"
    ...
    <div
      ...
      class="kx-rte min-h-32 px-3 py-2 text-sm text-foreground outline-none [&_h1]:text-xl [&_h1]:font-semibold [&_h2]:text-lg [&_h2]:font-semibold [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_a]:text-primary [&_a]:underline"
      @input="sync"
      @blur="sync"
    />
  </div>
