...
describe('useKinetixHotkeys matcher', () => {
    ...
    it('matches a plain key only without modifiers', () => {
        ...
        expect(eventMatchesStep(key({ key: 'c', ctrlKey: true }), 'c')).toBe(
            false,
        );
    });
    ...
    it('matches a mod combo (Ctrl or Cmd)', () => {
        expect(
            eventMatchesStep(key({ key: 'e', metaKey: true }), 'mod+e'),
        ).toBe(true);
        expect(
            eventMatchesStep(key({ key: 'e', ctrlKey: true }), 'mod+e'),
        ).toBe(true);
        expect(eventMatchesStep(key({ key: 'e' }), 'mod+e')).toBe(false);
    });

    it('requires shift when declared but allows it for symbol keys', () => {
        expect(eventMatchesStep(key({ key: '?', shiftKey: true }), '?')).toBe(
            true,
        );
        expect(
            eventMatchesStep(key({ key: 's', shiftKey: true }), 'shift+s'),
        ).toBe(true);
