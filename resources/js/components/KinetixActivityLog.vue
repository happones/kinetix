...
async function fetchPage(page: number): Promise<void> {
  ...
  entries.value = page === 1 ? result.data : [...entries.value, ...result.data];
  ...
  <div class="space-y-4">
    ...
    <div v-if="loading && entries.length === 0" class="space-y-3">
      <div
        v-for="n in 3"
        :key="n"
        class="h-12 w-full animate-pulse rounded-md bg-muted"
      />
    </div>
    ...
    <ol v-else class="space-y-3">
      ...
      >
        <div class="flex items-baseline justify-between gap-2">
          <span class="text-sm font-medium text-foreground">
            ...
            <span class="font-normal text-muted-foreground">
              {{
                entry.causerName
                  ? t("kinetix.activity_by", { name: entry.causerName })
        ...
        >
          <li v-for="(line, i) in changeList(entry)" :key="i" class="font-mono">
            {{ line }}
          </li>
        </ul>
      </li>
