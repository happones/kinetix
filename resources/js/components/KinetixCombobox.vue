  ...
  >
    ...
    <ComboboxPortal>
      <ComboboxContent
        position="popper"
        :side-offset="4"
        class="relative z-50 max-h-96 w-[--reka-combobox-trigger-width] min-w-[8rem] overflow-hidden rounded-md border border-border bg-popover text-popover-foreground shadow-md"
      >
        ...
        <ComboboxViewport class="max-h-60 overflow-y-auto p-1">
          <ComboboxItem
            ...
            :key="val"
            :value="String(val)"
            class="relative flex w-full cursor-default select-none items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm text-foreground outline-none data-[highlighted]:bg-accent data-[highlighted]:text-accent-foreground"
          >
            <span>{{ label }}</span>
            <span
