...
- **`TimePicker`** (type `time-picker`): time-only field storing `H:i`. `dateConfig()` → `useCalendar` (false via `->native()` → `<input type=time>`), `minuteStep()`, `twelveHour()`. Rendered by `KinetixTimePicker` (inline scrollable hour/minute(/AM-PM) columns, reusing the DateTimePicker time UI). Same `useCalendar` true/false branch pattern as DatePicker/DateTimePicker in `KinetixFormSchema`.
