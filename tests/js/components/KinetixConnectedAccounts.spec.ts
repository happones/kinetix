...
describe("KinetixConnectedAccounts", () => {
  it("lists providers and shows a connect link for unlinked ones", async () => {
    fetchMock.mockResolvedValueOnce({
      accounts: [],
      providers: [
        { key: "github", label: "GitHub", icon: "github", color: null, linked: false },
        { key: "google", label: "Google", icon: "google", color: null, linked: false },
      ],
      hasPassword: true,
    });
  ...
  it("shows the linked identity and a disconnect control", async () => {
    fetchMock.mockResolvedValueOnce({
      ...
      providers: [
        { key: "github", label: "GitHub", icon: "github", color: null, linked: true },
      ],
      hasPassword: true,
    });
