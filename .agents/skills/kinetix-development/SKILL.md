...
- **Host-model key columns (REQUIRED)**: columns referencing HOST models (`user_id`, `team_id`, morph ids, `*_by` ids) are typed `unsignedBigInteger` with an index and NO DB-level foreign key — hosts on UUID/ULID keys retype the published migration. When a new feature adds such a column: add the feature to the table in `docs/installation.md` § "UUID / ULID primary keys on your models" AND append the standard "UUID / ULID Host Models" note (with the feature's columns + publish tag) to its Boost skill. Columns referencing Kinetix's own tables stay bigint and are never listed.
