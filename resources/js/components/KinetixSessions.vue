...
import type { KinetixBrowserSession } from "@/types";
  ...
  <div class="space-y-4">
    ...
    <template v-else>
      <ul class="divide-y divide-border rounded-lg border border-border">
        ...
        >
          <span
            class="flex size-9 shrink-0 items-center justify-center rounded-md border border-border bg-muted/40 text-muted-foreground"
          >
            <component :is="deviceIcon(session.device)" class="size-5" />
          </span>
          ...
          <div class="min-w-0 flex-1">
            <p class="flex items-center gap-2 text-sm font-medium text-foreground">
              ...
              <span
                ...
                class="inline-flex items-center gap-1 rounded-full bg-success/15 px-2 py-0.5 text-xs font-medium text-success"
              >
            ...
            <p class="truncate text-xs text-muted-foreground">
              {{ session.ipAddress }}
              <template v-if="!session.isCurrentDevice && session.lastActive">
